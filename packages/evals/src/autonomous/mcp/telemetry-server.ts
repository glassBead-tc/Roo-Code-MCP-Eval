...
export class TelemetryMCPServer {
			...
			try {
				switch (name) {
					case "query_telemetry":
						return await this.handleQueryTelemetry(
							args as TelemetryQueryParams & { limit?: number; offset?: number },
						)
	...
	private async handleExportTelemetry(params: {
		format: string
		compression?: boolean
	...
	private getWindowInterval(window: string): number {
		switch (window) {
			case "5m":
				return 15000 // 15 seconds per point
			case "15m":
				return 45000 // 45 seconds per point
			case "1h":
				return 180000 // 3 minutes per point
			default:
				return 45000
		}
	}
