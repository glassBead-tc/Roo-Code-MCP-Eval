...
} from "../config/claude-config-generator.js"
...
program.name("autonomous-analysis").description("Autonomous MCP analysis system CLI").version("1.0.0")
		...
		try {
			...
			if (result.valid) {
				...
				if (result.testResults) {
					console.log(
						`🧪 Tests: ${result.testResults.unitTests.passed} passed, ${result.testResults.unitTests.failed} failed`,
					)
...
program
	...
	.action(async (options) => {
		try {
			...
			const { createDefaultConstraints, createDefaultSessionConfig } = await import(
				"../config/claude-config-generator.js"
			)
			...
			if (options.verbose) {
				...
				orchestrator.on("iteration-completed", (data) => {
					console.log(
						`Progress... Iteration ${data.iteration} completed (${data.result.recommendationsGenerated.length} recommendations)`,
					)
				})
			}
...
### By Server
${performance.byServer
	.map(
		(server: any) => `
- **${server.serverName}**: ${server.totalCalls} calls, ${server.avgResponseTime.toFixed(0)}ms avg, ${(server.errorRate * 100).toFixed(1)}% error rate`,
	)
	.join("")}

### By Task Type
${performance.byTaskType
	.map(
		(task: any) => `
- **${task.taskType}**: ${task.count} executions, ${(task.successRate * 100).toFixed(1)}% success rate`,
	)
	.join("")}
...
- **${pattern.pattern}**: ${pattern.frequency} occurrences (${pattern.severity} severity)
  ...
  - Suggested fix: ${pattern.suggestedFix || "No specific fix available"}`,
	)
	.join("")}
...
function extractSummary(telemetryData: any) {
	return {
		...
		recommendationCount:
			telemetryData.recommendations.immediate.length + telemetryData.recommendations.longTerm.length,
	}
}
