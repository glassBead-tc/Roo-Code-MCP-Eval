...
const defaultConstraints: OperatingConstraints = {
	...
	fileAccess: {
		readOnly: ["*.md", "package.json", "tsconfig.json", "CHANGELOG.md"],
		writeAllowed: ["packages/evals/src/**/*.ts", "packages/evals/src/**/*.js", "packages/evals/build-utils/**/*"],
		prohibited: ["node_modules/**/*", ".git/**/*", "dist/**/*", "build/**/*"],
	},
	resourceLimits: {
		maxMemoryMB: 1024,
		maxCpuPercent: 80,
...
program.name("orchestrator").description("Autonomous Analysis Orchestrator CLI").version("1.0.0")
...
function setupBasicLogging(orchestrator: AnalysisOrchestrator): void {
	...
	orchestrator.on("iteration-completed", (data) => {
		...
		console.log(
			`Progress${dots} Iteration ${data.iteration} completed (${data.result.recommendationsGenerated.length} recommendations)`,
		)
...
function displaySessionHistory(history: any[]): void {
	...
	history.forEach((session) => {
		...
		if (session.results?.length > 0) {
			const totalRecs = session.results.reduce(
				(sum: number, r: any) => sum + r.recommendationsGenerated.length,
				0,
			)
...
function displayDetailedMetrics(metrics: any): void {
	...
	const successRate =
		metrics.validationsPassed + metrics.validationsFailed > 0
