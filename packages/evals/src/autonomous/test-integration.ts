...
function createTestConfiguration(): OrchestratorConfig {
  ...
  const constraints: OperatingConstraints = {
    filePermissions: {
      readOnlyPaths: ['*.md', 'package.json'],
      writeAllowedPaths: ['packages/evals/src/**/*.ts'],
      prohibitedPaths: ['node_modules/**/*', '.git/**/*']
    },
    ...
    testingRequirements: {
      runTestsBeforeChange: false, // Disable for testing
      runTestsAfterChange: false,  // Disable for testing
